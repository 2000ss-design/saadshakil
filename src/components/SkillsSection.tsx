      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {skillCategories.map((cat, ci) => (
            <div
              key={cat.title}
              className={`glass rounded-xl p-4 sm:p-6 glow-border hover:border-primary/40 transition-all duration-500 group hover:-translate-y-2 hover:shadow-[0_0_30px_hsl(var(--primary)/0.15)] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              style={{ transitionDelay: `${ci * 120}ms` }}
            >
              <div className="text-xl sm:text-2xl mb-2">{cat.icon}</div>
              <h3 className="text-primary font-semibold mb-3 sm:mb-4 font-mono text-xs sm:text-sm tracking-wider">{cat.title}</h3>
